...
**前端调用顺序**：`GET /ai/agent/list` → 用户选择智能体 → `GET /ai/agent/schema?agent_id=xxx` → 渲染 3D 图 → 用户点击运行 → `POST /ai/agent/run`（传 `agent_id`、`input`）→ 用返回的 `executionOrder`、`steps` 驱动步骤动画，`finalState.response` 展示最终回答。
...
- `data` 的 key 为 `agent_id`，用于后续 schema、run 请求；每个 value 需含 `id`、`name`、`description`、`type`、`icon`。
- `type`：`deliberative` | `reactive` | `hybrid`，前端用于展示标签（深思熟虑/反应式/混合型）。
- **前端兜底**：若接口失败或不可用，前端使用内置 `AGENT_LIST` 仍可展示智能体列表，仅无法从服务端动态更新。
...
- 前端用 `data`（即 `nodes` + `edges`）绘制 3D 图；切换智能体时只更新 `graphData` 传参、不 remount 可视化组件，避免背景/Canvas 重新加载。
...
| 字段         | 类型   | 必填 | 说明 |
|--------------|--------|------|------|
| `agent_id`   | string | 是*  | 当前选中的智能体 id；前端每次运行都会传 |
| `input`      | object | 否   | 与具体 Agent 对应，不传则后端使用该 Agent 默认输入 |
| `stream`     | boolean| 否   | 默认 `false`；为 `true` 时走 SSE，见 2.4 |

\* 后端若支持默认值可为“否”，前端实现中为必传。
...
- **步骤**：用 `data.steps` 与 `data.executionOrder` 驱动 3D 步骤动画；`data.steps[].nodeId`、`duration_ms`、`output` 与 `steps[].label` 用于高亮与说明。
- **进度**：`data.totalSteps` 为总步数，当前步为 `stepIndex + 1`，进度 = `(stepIndex + 1) / totalSteps * 100%`。
- **3D 可视化挂载**：切换智能体时仅更新 `graphData` 传参，不因 `agent_id` 变化而 remount 可视化组件（前端使用稳定 key `schema`），避免 Canvas/背景重新加载；仅在「有运行结果」时用 `runKey` 触发重挂载以播放步骤动画。

...
import { SoundOutlined, ToolOutlined } from '@ant-design/icons'
import {
  fetchTtsAgentInfo,
  ttsChatStream,
  type TtsAgentInfo,
  type ChatMessage,
} from '../../service/mcp-tts'
import './MCPGaode.css'
...
class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  componentDidCatch(error: Error) {
    console.error('[MCP TTS Error]', error)
  }
...
export default function MCPTts() {
  const [info, setInfo] = useState<TtsAgentInfo | null>(null)
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      if (info?.config_required) {
        message.warning('请先配置 TTS MCP 后再使用')
        return
      }
      ...
      try {
        const nextHistoryFromStream = await ttsChatStream(nextHistory, {
          onStep: (stepMessages) => {
          ...
          <div className="mcp-gaode-sidebar-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <Tag color="green" style={{ margin: 0 }}>语音合成</Tag>
              </div>
            </div>
            <p className="mcp-gaode-desc">{info?.description ?? '将文字转换为语音，支持多种语言和声音'}</p>
            {info?.config_required && (
              <Alert
                type="warning"
                showIcon
                message="未配置"
                description={info?.config_hint ?? '请配置 TTS MCP 相关参数'}
                style={{ marginTop: 12 }}
              />
            )}
          </div>
            ...
            {messages.length === 0 && (
              <div className="mcp-gaode-empty">
                <div className="mcp-gaode-empty-row">
                  <SoundOutlined className="mcp-gaode-empty-icon" />
                  <span className="mcp-gaode-empty-text">文字转语音助手</span>
                </div>
                <p className="mcp-gaode-empty-hint">输入文字内容，我会帮你转换为语音</p>
              </div>
            )}
            {messages.map((msg, i) => (
            ...
            {sending && messages.length > 0 && messages[messages.length - 1]?.role === 'user' && (
              <div className="mcp-gaode-row mcp-gaode-row-assistant">
                <div className="mcp-gaode-bubble mcp-gaode-bubble-assistant mcp-gaode-typing">
                  <span className="mcp-gaode-typing-label">正在生成语音，请稍候…</span>

...
import { SunOutlined, ToolOutlined } from '@ant-design/icons'
import {
  fetchWeatherAgentInfo,
  weatherChatStream,
  type WeatherAgentInfo,
  type ChatMessage,
} from '../../service/mcp-weather'
import './MCPGaode.css'
...
function toolDisplayName(toolName: string): string {
  return toolName.replace(/^weather-/, '')
}
...
export interface MCPWeatherProps {
  inputTopSlot?: React.ReactNode
}

export default function MCPWeather(props: MCPWeatherProps = {}) {
  const { inputTopSlot } = props
  const [info, setInfo] = useState<WeatherAgentInfo | null>(null)
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      if (info?.config_required) {
        message.warning('请先配置天气 MCP 后再使用')
        return
      }
      ...
      try {
        const nextHistoryFromStream = await weatherChatStream(nextHistory, {
          onStep: (stepMessages) => {
          ...
          <div className="mcp-gaode-sidebar-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <Tag color="cyan" style={{ margin: 0 }}>天气</Tag>
              </div>
            </div>
            <p className="mcp-gaode-desc">{info?.description ?? '接入 DashScope 天气 MCP，支持城市天气查询'}</p>
            {info?.config_required && (
              <Alert
                type="warning"
                showIcon
                message="未配置"
                description={info?.config_hint ?? '请设置 MCP_WEATHER_URL 与 MCP_WEATHER_API_KEY 或 DASHSCOPE_API_KEY'}
                style={{ marginTop: 12 }}
              />
            )}
          </div>
            ...
            {messages.length === 0 && (
              <div className="mcp-gaode-empty">
                <div className="mcp-gaode-empty-row">
                  <SunOutlined className="mcp-gaode-empty-icon" />
                  <span className="mcp-gaode-empty-text">查城市天气</span>
                </div>
                <p className="mcp-gaode-empty-hint">输入城市名或问题，如「北京今天天气怎么样」</p>
              </div>
            )}
            {messages.map((msg, i) => (
            ...
            {sending && messages.length > 0 && messages[messages.length - 1]?.role === 'user' && (
              <div className="mcp-gaode-row mcp-gaode-row-assistant">
                <div className="mcp-gaode-bubble mcp-gaode-bubble-assistant mcp-gaode-typing">
                  <span className="mcp-gaode-typing-label">正在查询天气…</span>
